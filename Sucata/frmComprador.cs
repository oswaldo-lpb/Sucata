...
namespace Sucata
{
    public partial class frmComprador : Form
    {
        ...
        {
            nmCompradorTextBox.Enabled = true;
        ...
        {
            nmCompradorTextBox.Enabled = false;
        ...
        {
            if (tbCompradorBindingSource.Count > 0)
            {
                if (MessageBox.Show("Confirma exclusão?", "Excluir Comprador", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                {
                    tbCompradorBindingSource.RemoveCurrent();
                    Validate();
                    tbCompradorTableAdapter.Update(bDSucataDataSet.tbComprador);
                }
            }
        }
        ...
        {
            tbCompradorBindingSource.MovePrevious();
        }

        private void btnProximo_Click(object sender, EventArgs e)
        {
            tbCompradorBindingSource.MoveNext();
        }

        private void btnNovo_Click(object sender, EventArgs e)
        {
            tbCompradorBindingSource.AddNew();
            vlSaldoTextBox.Text = string.Format("{0:N2}", 0);
            HabilitaEdicao();
            nmCompradorTextBox.Focus();
        }
        ...
        private void btnSalvar_Click(object sender, EventArgs e)
        {
            ...
            {
                Validate();
                tbCompradorBindingSource.EndEdit();
                tbCompradorTableAdapter.Update(bDSucataDataSet.tbComprador);
                DesabilitaEdicao();
            }
        }
        ...
        {
            tbCompradorBindingSource.CancelEdit();
            DesabilitaEdicao();
        }
        ...
        {
            int cod = 0, reg;
            frmCompradorPesquisa fcp = new frmCompradorPesquisa();
            fcp.ShowDialog();
            cod = fcp.getCodigo();
            if (cod > 0)
            {
                reg = tbCompradorBindingSource.Find("cdComprador", cod);
                tbCompradorBindingSource.Position = reg;
            }
        }
        ...
        private void nmCompradorTextBox_Enter(object sender, EventArgs e)
        {
        ...
        private void nmCompradorTextBox_Leave(object sender, EventArgs e)
        {
            if (sender is TextBox)
            {
        ...
        private void frmComprador_KeyDown(object sender, KeyEventArgs e)
        {
        ...
        private void printDocument1_PrintPage(object sender, System.Drawing.Printing.PrintPageEventArgs e)
        {
            ...
            strDados = "Ficha de Comprador" + (char)10 + (char)10;
            strDados = strDados + "Código: " + cdCompradorTextBox.Text + (char)10 + (char)10;
            strDados = strDados + "Nome: " + nmCompradorTextBox.Text + (char)10 + (char)10;
